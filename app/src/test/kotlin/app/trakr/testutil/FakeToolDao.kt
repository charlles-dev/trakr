...
class FakeToolDao : ToolDao {
    ...
    override fun observeScansPerDay(days: Int): Flow<List<DayCount>> =
        scanSessions.map { sessions ->
            sessions.groupBy { java.text.SimpleDateFormat("yyyy-MM-dd").format(java.util.Date(it.ts)) }.map {
                    (day, list) ->
                DayCount(day, list.size)
            }.sortedByDescending { it.day }.take(days)
        }

    override fun observeMostForgotten(limit: Int): Flow<List<MostForgotten>> =
        alerts.map { list ->
            list.groupBy { it.toolId }.map {
                    (id, items) ->
                MostForgotten(id, items.firstOrNull()?.toolName ?: id, items.size)
            }.sortedByDescending { it.cnt }.take(limit)
        }

    override fun observeAlertsPerDay(): Flow<List<DayCount>> =
        alerts.map { list ->
            list.groupBy { java.text.SimpleDateFormat("yyyy-MM-dd").format(java.util.Date(it.createdAt)) }.map {
                    (day, items) ->
                DayCount(day, items.size)
            }.sortedByDescending { it.day }
        }

    override fun observeLongestAbsent(limit: Int): Flow<List<Tool>> =
        tools.map { list ->
            list.filter {
                !it.present && it.lastSeenAt != null
            }.sortedBy { it.lastSeenAt }.take(limit)
        }

    override fun observePresenceRate(): Flow<Float?> =
        tools.map { list ->
    ...
    override suspend fun clearTools() {
        tools.value = emptyList()
    }

    override suspend fun clearRssiSamples() {
        rssiSamples.value = emptyList()
    }

    override suspend fun clearToolSettings() {
        toolSettings.value = emptyList()
    }
