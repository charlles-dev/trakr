...
O Trakr é alimentado por uma **bateria 18650** (carregada via TP4056, USB-C).
...
> ESP32 entra em `esp_deep_sleep_start()` com wake-up ext0 no Sensor Hall.
> Falta apenas **medir** e registrar os números na tabela abaixo.
...
- Celular com o app Trakr (para forçar os estados BLE/ALERTA);
...
| --- | --- | --- | --- | --- |
| **DORME** (deep sleep) | Fechar tampa + aguardar ~30 s | ~10–40 µA | ~1 µA (EN LOW) | **~50 µA** |
| **LEITURA** (~500 ms) | Abrir tampa (wake) | 200–300 mA | 150–250 mA | ~400 mA (pico) |
| **MONITOR** (5 s) | Logo após varredura | ~80 mA | off | ~80 mA |
| **ALERTA** (3 s) | Retirar ferramenta | ~90 mA (buzzer/LED) | off | ~120 mA |
...
Exemplo ilustrativo com **18650 de 3000 mAh** e 2 aberturas/dia (~31 s ativo):
...
- Se o consumo em DORME ficar acima de ~100 µA, investigue antes de fechar:
  ...
  2. BUZZER não drenado (`alarmOff()` antes do sleep);
